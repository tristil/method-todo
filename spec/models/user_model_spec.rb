...
describe User do
  it 'should require username, email and password' do
    User.new(username: 'Example',
             email: 'example1@example.com').should have(1).error_on(:password)
    User.new(email: 'example2@example.com',
             password: 'Password1').should have(1).error_on(:username)
    User.new(username: 'Example',
             email: 'example2@example.com',
             password: 'Password1').should have(0).errors
  end
  ...
  it '.active_todos should return non-completed todos' do
    ...
    user.reload
    user.todos.should eq [todo, todo2]
    user.active_todos.should eq [todo]
  end
  ...
  it '.completed_todos should return completed todos' do
    ...
    user.reload
    user.todos.should eq [todo, todo2]
    user.completed_todos.should eq [todo2]
  end
  ...
  it '.destroy should use Acts as Paranoid to virtually delete the user' do
    ...
    user.destroy
    User.all.should eq []
    User.only_deleted.should eq [user]
  end
  ...
  it '.todo_contexts should return contexts for this user' do
    ...
    user.save
    user.reload
    user.todo_contexts.should eq [todo_context]
  end
  ...
  it '.preferences should return and set a hash' do
    ...
    user.save
    user.reload
    user.preferences[:show_help].should eq false
  end
